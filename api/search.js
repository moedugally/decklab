...
- "damage based on opponent's damage counters" / "more damage if opponent's pokemon is already hurt" / "damage scales with damage counters on opponent's active" / "bonus damage if opponent's pokemon is damaged" → cardTextContains: ["already has any damage counters on it", "for each damage counter on your opponent's Active", "for each damage counter on the Defending Pokémon", "damage counters on your opponent's Active Pokémon", "Benched Pokémon that has any damage counters on it"]. Only phrases referencing "your opponent's" or "the Defending Pokémon" qualify.
...
export default async function handler(req, res) {
  ...
  const cacheKey = `v68:search:standard:${typeFilter.toLowerCase()}:${query.trim().toLowerCase()}`;
