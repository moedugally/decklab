...
- "pokemon that hit all pokemon with damage counters" / "damage to all pokemon that are already damaged" / "spread damage to injured pokemon" → cardTextContains: ["has any damage counters on it", "damage counters on it (both yours and your opponent's"]
...
export default async function handler(req, res) {
  ...
  const cacheKey = `v66:search:standard:${typeFilter.toLowerCase()}:${query.trim().toLowerCase()}`;
